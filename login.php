...
<body class="min-h-screen flex items-center justify-center bg-gray-100">
  ...
  <div class="card w-full max-w-md bg-base-100 shadow-xl mx-4">
    <div class="card-body">
      ...
      <?php if ($action === 'login' || $action === 'register') : ?>
        <form class="flex flex-col gap-4 mt-4" action="?action=<?= htmlspecialchars($action) ?>" method="post">
          ...
          <input type="submit" class="btn btn-active btn-primary" value="<?= $action === 'login' ? 'Login' : 'Register' ?>" name="<?= $action ?>">
        </form>
      <?php endif; ?>
    </div>
  </div>
